...
for i in {1..5}; do yum -y install https://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm && break || sleep 5; done

# install jq
for i in {1..5}; do yum -y install jq && break || sleep 5; done
...
do
  ...
  if [ -f "./install.bash" ]; then
    ...
    bash ./install.bash agent:certname=$fqdn agent:environment=$environment extension_requests:pp_role=$role extension_requests:pp_environment=$environment extension_requests:pp_hostname=$fqdn extension_requests:pp_zone=$zone
    sleep 20
    break
  elif [ "$n" -lt "$t" ]; then
