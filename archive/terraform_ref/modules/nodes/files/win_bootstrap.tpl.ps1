...
:loop while ($true) {
  ...
  if (Test-Path "C:\pe_install.ps1") {
    ...
    Invoke-Expression -Command "C:\pe_install.ps1 agent:certname=$FQDN agent:environment=$ENVIRONMENT extension_requests:pp_role=$ROLE extension_requests:pp_environment=$ENVIRONMENT extension_requests:pp_hostname=$FQDN extension_requests:pp_zone=$ZONE"
